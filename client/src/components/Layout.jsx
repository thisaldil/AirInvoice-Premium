  ...
  return (
    <div className="flex h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white dark:bg-gray-800 shadow px-4 py-3 flex justify-between items-center">
        ...
        <img src={darklogo} alt="logo" className="h-10 hidden dark:block" />
      ...
      {/* Sidebar */}
      <div
        className={`fixed pt-10 md:pt-0 md:relative z-40 top-0 left-0 h-full w-64 bg-white dark:bg-gray-800 shadow-md transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"
          } md:translate-x-0`}
      >
            ...
            {menuItems.map((item) => (
              <li key={item.path}>
                <button
                  ...
                  className={`flex items-center w-full px-6 py-3 text-left transition-all ${location.pathname === item.path
